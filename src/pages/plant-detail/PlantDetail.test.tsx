...
describe("PlantDetail Component", () => {
  ...
  it("renders correctly", async () => {
    expect(screen.getByTestId("plant-detail")).toBeInTheDocument();
  });

  it("when component is load, it called plantsDetail api should be called", async () => {
    await waitFor(() => expect(mockPlantDetail).toHaveBeenCalledTimes(1));
  });

  it("when user clicks on plant button, it redirect to the plant-list URL", async () => {
    await act(async () => {
