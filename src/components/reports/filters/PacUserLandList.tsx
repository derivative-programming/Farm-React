...
const ReportFilterPacUserLandList: FC<ReportFilterPacUserLandListProps> = ({
  ...
  const validationSchema = PacUserLandListReportService.buildValidationSchema();
  ...
  const submitButtonClick = async (
    ...
    actions: FormikHelpers<PacUserLandListReportService.QueryRequest>
  ) => {
    try {
      setLoading(true);
  ...
  const onAccordianHeaderClick = () => {
    logClick("ReportFilterPacUserLandList","accordianClick","");
      ...
      <Accordion defaultActiveKey={defaultAccordianKey} alwaysOpen={!isCollapsible}>
        <Accordion.Item eventKey="0">
          ...
          <Accordion.Body>
            ...
            >
              {(props: FormikProps<PacUserLandListReportService.QueryRequest>) => (
                <Form
                  name={name}
                  data-testid={name}
