...
export interface ReportFilterPacUserRoleListProps {
  name: string;
...
const ReportFilterPacUserRoleList: FC<ReportFilterPacUserRoleListProps> = ({
  name,
  initialQuery,
  onSubmit,
  hidden = false,
  ...
  const validationSchema = PacUserRoleListReportService.buildValidationSchema();
  ...
  const submitButtonClick = async (
    ...
    actions: FormikHelpers<PacUserRoleListReportService.QueryRequest>
  ) => {
    try {
      setLoading(true);
      logClick("ReportFilterPacUserRoleList","submit","");
      onSubmit(values);
    }
    finally {
  ...
  const onAccordianHeaderClick = () => {
    logClick("ReportFilterPacUserRoleList","accordianClick","");
      ...
      <Accordion defaultActiveKey={defaultAccordianKey} alwaysOpen={!isCollapsible}>
        <Accordion.Item eventKey="0">
          ...
          <Accordion.Body>
            ...
            >
              {(props: FormikProps<PacUserRoleListReportService.QueryRequest>) => (
                <Form
                  name={name}
                  data-testid={name}
...
export default ReportFilterPacUserRoleList;
