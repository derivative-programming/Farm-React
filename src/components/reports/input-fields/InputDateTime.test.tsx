...
import {ReportInputDateTime} from "./InputDateTime";   
import { Formik } from "formik";
import { Form } from "react-bootstrap";

const initialValues = { testName:"" } 
 
describe("ReportInputDateTime Component", () => {
  // render the ReportInputDateTime component
  beforeEach(() => {
    render(
      <Formik
      ...
      <ReportInputDateTime label="Test Label" name="testName"/> 
              </Form>  
          )}
      </Formik>
    );
  ...
  it("when user sets prop autoFocus to true, control is autoFocused", async () => {
    render( 
      <Formik
          ...
          {(props) => (
              <Form onReset={props.handleReset} onSubmit={props.handleSubmit}> 
      <ReportInputDateTime label="Test Label" name="testName2" autoFocus={true}/> 
              </Form>  
          )}
      </Formik>
    );
