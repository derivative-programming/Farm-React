...
import {ReportInputNumber} from "./InputNumber";   
import { Formik } from "formik";
import { Form } from "react-bootstrap";

const initialValues = { testName:"" } 
 
describe("ReportInputNumber Component", () => {
  // render the ReportInputNumber component
  beforeEach(() => {
    render(
      <Formik
      ...
      <ReportInputNumber label="Test Label" name="testName"/> 
      </Form>  
  )}
</Formik>
    );
  ...
  it("when user sets prop autoFocus to true, control is autoFocused", async () => {
    render( 
      <Formik
          ...
          {(props) => (
              <Form onReset={props.handleReset} onSubmit={props.handleSubmit}> 
      <ReportInputNumber label="Test Label" name="testName2" autoFocus={true}/> 
      </Form>  
  )}
</Formik>
    );
