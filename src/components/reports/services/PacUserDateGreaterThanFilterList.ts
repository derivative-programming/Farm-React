...
import * as ReportInit  from "./init/PacUserDateGreaterThanFilterListInitReport";
import { apiCall } from "../../../apiConfig/apiCall";

export const submitRequest = (data:QueryRequest, pacCode:string) => {
    return apiCall({
      url:  "/pac-user-date-greater-than-filter-list/" + pacCode,
      method: "get",
      params: data
    });
  };
...
export const submitCSVRequest = (data:QueryRequest, pacCode:string) => {
  console.log('csv request');
    return apiCall({
      url:  "/pac-user-date-greater-than-filter-list/" + pacCode + "/to-csv",
      method: "get",
      params: data
    });
  };
  ...
  export const initPage = (pacCode:string) => {
    const data = {};
    return apiCall({
      url: "/pac-user-date-greater-than-filter-list/" + pacCode + '/init',
      method: "get",
      data
    });
  };
