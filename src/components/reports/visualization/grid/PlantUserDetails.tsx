...
import { QueryResultItem } from "../../services/PlantUserDetails"; // NOSONAR
...
export interface ReportGridPlantUserDetailsProps {
  ...
  items: PlantUserDetailsReportService.QueryResultItem[];
...
export const ReportGridPlantUserDetails: FC<ReportGridPlantUserDetailsProps> = ({
  ...
  const componentName = "ReportGridPlantUserDetails";
  ...
  const onSelectAllRows = (e: React.ChangeEvent<HTMLInputElement>) => {  //NOSONAR
    if (e.target.checked) {
      ...
      setCheckedIndexes(
        items.map((item: PlantUserDetailsReportService.QueryResultItem, index) =>
          index.toString()
        )
      );
    } else {
