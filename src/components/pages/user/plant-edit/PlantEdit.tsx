...
import { 
  pacUserFlavorListService,
  plantEditService,
  plantUserDetailsService,
} from "../../../../services/home";
import {
  Formik,
  Form as FormikForm,
...
const PlantEdit: FC = (): ReactElement => {
  ...
  const [plantUserDetails, setPlantUserDetails] = useState(initialValues);

  const getInitialValues = () => {
    if (params && params["id"] && plantUserDetails) {
      console.log(plantUserDetails);
      const test: PlantValue = {
        flavorCode: plantUserDetails["flavorCode"],
        otherFlavor: plantUserDetails["otherFlavor"],
        someIntVal: plantUserDetails["someIntVal"],
        someBigIntVal: plantUserDetails["someBigIntVal"],
        someBitVal: plantUserDetails["someBitVal"],
        isEditAllowed: plantUserDetails["isEditAllowed"],
        isDeleteAllowed: plantUserDetails["isDeleteAllowed"],
        someFloatVal: plantUserDetails["someFloatVal"],
        someDecimalVal: plantUserDetails["someDecimalVal"],
        someUTCDateTimeVal: moment(plantUserDetails["someUTCDateTimeVal"]),
        someDateVal: moment(plantUserDetails["someDateVal"]),
        someMoneyVal: plantUserDetails["someMoneyVal"],
        someNVarCharVal: plantUserDetails["someNVarCharVal"],
        someVarCharVal: plantUserDetails["someVarCharVal"],
        someTextVal: plantUserDetails["someTextVal"],
        somePhoneNumber: plantUserDetails["somePhoneNumber"],
        someEmailAddress: plantUserDetails["someEmailAddress"],
        sampleImageUploadFile: plantUserDetails["sampleImageUploadFile"],
      };
      return test;
    } else {
  ...
  const getPlantUserDetails = async (id: string) => {
    try {
      const res = await plantUserDetailsService(id);
  ...
  const getFlavorList = async () => {
    try {
      const res = await pacUserFlavorListService();
      if (res?.data?.items) {
...
var x = 1;
    ...
    plantEditService(data, params["id"])
        .then((res: any) => {
          console.log(res);
          if (res && res["data"] && res["data"]["success"]) {
            goTo("/land-plant-list");
          }
        })
        .catch((err) => {
          console.log(err);
  ...
  return (
    <div className="plant-edit-container" data-testid="plant-edit-container">
      <Card>
        <h1>{params && params["id"] ? "Update Plant" : "Update Plant"}</h1>
