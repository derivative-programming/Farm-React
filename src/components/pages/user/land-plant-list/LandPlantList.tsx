...
import { landPlantListService, plantDeleteService, pacUserFlavorListService } from "../../../../services/home";
...
const LandPlantList: FC = (): ReactElement => {
  ...
  const sort = (key: string) => {
    ...
    setLandPlantList(orderBy(landPlantList, key, order))
    ...
    if (value) {
      ...
      if (deleteCode) {
        ...
        for (let i = 0; i < selectedItem.length; i++) {
          plantDeleteService(selectedItem[i])
            .then((res) => {
              console.log(res);
  ...
  const getLandPlantList = async () => {
    try {
      const res = await landPlantListService(config, params["id"]);
      console.log("landPlantListService res -->", res);
  ...
  const getFlavorList = async () => {
    try {
      ...
      console.log("pacUserFlavorListService res -->", res);
      if (res?.data?.items) {
  ...
  const onEditPlant = (item: any) => {
    goTo("/plant-edit/" + item["plantCode"]);
  };
  const onViewPlant = (item: any) => {
    goTo("/plant-user-details/" + item["plantCode"]);
  };
  ...
  const onSelectAll = (e: any) => {
    let selectedItems = cloneDeep(selectedItem);
    for (let i = 0; i < landPlantList.length; i++) {
      let index = selectedItems.indexOf(landPlantList[i]['plantCode']);
      if (e.target.checked) {
        if (index === -1) {
          selectedItems.push(landPlantList[i]['plantCode']);
        }
      }
      else {
        if (index !== -1) {
  ...
  const isCheckedAll = () => {
    ...
    for (let index = 0; index < landPlantList.length; index++) {
      if (!selectedItem.includes(landPlantList[index]['plantCode'])) {
        flag = false;
        break;
      }
    }
    ...
    <div className="report-container" data-testid="land-plant-list">
      ...
      <div className="report-list-button-body">
        ...
        >
          ...
          <tbody>
            {landPlantList && landPlantList.length
              ? landPlantList.map((item: PlantsValue, index) => {
                return (
