...
import TacRegister from "./TacRegister";
...
describe("TacRegister Component", () => {
  // api mock
  const mockTacRegisterService = jest.spyOn(authController, "tacRegisterService");
  ...
  it("renders correctly", async () => {
    expect(screen.getByTestId("tacRegister")).toBeInTheDocument();
  });
  ...
  it("when user entered tacRegister details and clicks on register button, tacRegisterUser api should be called", async () => {
    mockTacRegisterService.mockResolvedValueOnce({
      data: tacRegisterResponse,
    });
    ...
    await waitFor(() => expect(mockTacRegisterService).toHaveBeenCalledTimes(1));
  });
});
